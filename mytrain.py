...
from ocTree import insertJ, Octree, insertJP, avDensity
...
def config_parser():
    ...
    parser.add_argument("--expname", type=str, default='phase-exp-1',
                        help='experiment name')
    parser.add_argument("--basedir", type=str, default='./logs/',
                        help='where to store ckpts and logs')
    parser.add_argument("--datadir", type=str, default='./data/phase-exp-1/',
...
def get_points(octree, rays_o, rays_d, near, far, n_samples):
    ...
    # sample points
    # 根据每一组pp，sp采样（点数不同）
    # 怎么把各组之间不同数量的采样点输入到网络里？
    rays_or = tr.reshape(rays_o, [-1, rays_o.shape[-1]])
    rays_dr = tr.reshape(rays_d, [-1, rays_d.shape[-1]])
    pA = []
    pnum = []
    tt1 = time.time()
    for o, d in zip(rays_or, rays_dr):
        if len(pnum) < 1:
            lasN = 0
        else:
            lasN = len(pA)
        # print(far-near)
        pA = insertJP(octree.root, o.cpu(), d.cpu(), n_samples, pA, all_d)
        pnum.append(len(pA) - lasN)
    tt2 = time.time()
    # print('insertJP time consuming:', tt2-tt1)
    pA = np.array(pA)
    # rays_o = rays_o.to(device)
    # rays_d = rays_d.to(device) # [batchsize, chunk, 3]
...
def render_signal_sp(octree, tags, rays_o, near, far, N_samples, **kwargs):
    ...
    for i in range(chunks_num):
        ...
        pts, pn = get_points(octree, rays_o_chunks, rays_d_chunks, 0, 0.3, nS)  # [bs*pts, 3]
        F = True
        for view, tag, n in zip(rayy, tags, pn):
            view = tr.reshape(view, [-1, view.shape[-1]])
            tag = tr.reshape(tag, [-1, tag.shape[-1]])
            view_chunk = tr.repeat_interleave(view, n, 0)
            tag_chunk = tr.repeat_interleave(tag, n, 0)
            if F:
                views_chunks = view_chunk
                tags_chunks = tag_chunk
                F = False
            else:
                views_chunks = tr.cat([views_chunks, view_chunk], 0)
                tags_chunks = tr.cat([tags_chunks, tag_chunk], 0)
            # print(view_chunk.shape)
        # views_chunks = rays_d_chunks[..., None, :].expand(pts.shape)  # [bs, cks, pts, 3]
        # tags_chunks = tags_chunk[..., None, :].expand(pts.shape)  # [bs, cks, pts, 3]

        raw = run_network(tr.tensor(pts), views_chunks, tags_chunks, **kwargs)  # [bs, cks, pts, 3]
        recv_signal_chunks, total_signal = raw2outputs_signal(raw, rays_d_chunks, nS)  # [bs]

        recv_signal -= recv_signal_chunks
        total += total_signal
    return recv_signal, total  # [batchsize,]

def render_signal(octree, rays_o, azs, els, near, far, N_samples, **kwargs):
    """_summary_
...
def raw2outputs_signal(raw, rays_d, nS):
    ...
    recv_signal = tr.sum(amp_i*s_a, -1)  # integral along line [batchsize,chunks]
...
def getSPDensity(**kwargs):
    ...
    y_nSamples = np.linspace(0, 1, yNum)
    z_nSamples = np.linspace(0, 0.3, zNum)
    SP = []
    densityArray = []
    for x in x_nSamples:
        for y in y_nSamples:
            ...
            # print(al)
            if al > 0.9:
                densityArray.append(1)
            else:
                densityArray.append(0)
            SP = []
    # s = raw[..., 1]
    # print(alphac.shape)
    # for al in alphac:
...
def train():
    ...
    octree.findLeaf(octree.root)
    # octree.extendNode(octree.root)
    dt = pd.read_csv("data/phase-exp-1/dataTrain.csv")
    dtV = pd.read_csv('data/phase-exp-1/dataEval.csv')
    dt.head()
    ...
    plt.ion()
    mapgt = []
    mapd = pd.read_csv("data/phase-exp-1/mapgt.csv")
    mapdt = mapd.values
    mapdt = np.reshape(mapdt,(-1))
    mapdt = mapdt.tolist()
    # print(mapdt.shape)
    ...
    for epoch in range(epoch_start, epoches + 1):
        ...
        for i in trange(iter_per_epoch):
            ...
            sPosition = batchdata[..., 3:6]
            # az = batchdata[..., 3]
            # el = batchdata[..., 4]
            # print(az)
            # predict_ss, predict_ts = render_signal(sPosition, az,el, **render_kwargs_train)
            # predict_ss, to = render_signal(octree, sPosition, az, el, **render_kwargs_train)
            predict_ss, to = render_signal_sp(octree, sPosition, pPosition, **render_kwargs_train)

            optimizer.zero_grad()
            # print("predict: ", predict_ss, "GT: ", cn0)
            dA = getSPDensity(**render_kwargs_train2)
            # print(len(mapdt))
            c = [mapdt[i] - dA[i] for i in range(len(mapdt))]
            print(c)
            mapLoss = sum(c)/32*32
            # print(len(dA))
            # map_loss = mapgt - dA
            loss = a * sig2mse(predict_ss, cn0) + (1 - a) * mapLoss
        ...
        with tr.no_grad():
            for i in trange(Viter_per_epoch):
                ...
                predict_ss, to = render_signal_sp(octree, sPosition, pPosition, **render_kwargs_train)
                dA = getSPDensity(**render_kwargs_train2)
                c = [mapdt[i] - dA[i] for i in range(len(mapgt))]
                mapLoss = sum(c) / 32*32
                loss = a * sig2mse(predict_ss, cn0) + (1 - a) * mapLoss
